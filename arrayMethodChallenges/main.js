...
// let desc = integers.sort((a, b) => b - a)
...
// let lessThanNineteen = desc.filter(number => number < 19);
// console.log('lessthan19', lessThanNineteen);

// // Multiply each remaining number by 1.5 and then subtract 1.

// let result = lessThanNineteen.map(number => number * 1.5 - 1);
// console.log('result', result);

// // Then output(either in the DOM or the console) the sum of all the resulting numbers.

// document.getElementById('output').innerText = result.reduce((total, nextNum) => total + nextNum);

//ALL ONE LINE:
document.getElementById('output').innerText = integers.sort((a, b) => b - a).filter(number => number < 19).map(num => num * 1.5 - 1).reduce((total, nextNum) => total + nextNum);
...
let Cars2017 = carData.filter((carObj => carObj.purchase_date.includes('2017')));
console.log('cars2017', Cars2017);

let ProfitsOnly = Cars2017.map(car => car.gross_profit);

let profit2017 = ProfitsOnly.reduce((total, nextProfit) => {
  return total + nextProfit;
})

console.log("2017 profit", profit2017);

// In which month did they sell the most cars ?

let purchaseDates = carData.map(car => car.purchase_date);
console.log('purchase dates', purchaseDates);

let months = purchaseDates.map((date => date.slice(5, 7)));
